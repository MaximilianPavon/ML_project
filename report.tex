...
\title{Methodology discussion for skewed classes data modelling \\ 
...
\begin{abstract}
In this project we explore different methods ranging from Logistic Regression and Support Vector Machines to Naïve Bayes classifiers and Neural Networks to tackle a simple Machine Learning problem.
...
We conclude that simple modelling is not sufficient enough and, albeit complicated, there are still opportunities for further enhancements regarding performance.
...
Ultimately an intuitive way to visualise the performance of the classification on the training data is a confusion matrix which is displayed in Figure~ \ref{pic:cm_logloss}, \ref{pic:cm_SVM}, 
%\ref{pic:cm_NB}
and \ref{pic:cm_NN} for Logistic Regression, Support Vector Machines, Naïve Bayes Classifiers and Neural Networks respectively.
